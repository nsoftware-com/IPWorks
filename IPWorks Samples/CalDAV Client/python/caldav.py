...
if len(sys.argv) < 3:
    ...
    print("  clientsecret  OAuth Client Secret")
    print("  username      the username to login")
    print("\r\nExample: python cal_client.py clientid clientsecret username")
else:
    try:
        oauth.set_client_id(sys.argv[1])
        oauth.set_client_secret(sys.argv[2])
        ...
        while True:
            ...
            if command == "1":
                ...
                caldav.create_event("https://apidata.googleusercontent.com/caldav/v2/" + caldav.get_user() + "/events/" + caldav.get_uid() + ".ics")
                ...
                if index > len(resource_uris):
                    ...
                    continue
                caldav.set_authorization(oauth.get_authorization())
                caldav.delete_event(resource_uris[index - 1])
