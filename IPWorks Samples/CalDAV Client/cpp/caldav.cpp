	...
	else
	{
		...
		while (true)
		{
			...
			else if (!strcmp(argument, "2"))     //Add Event
			{
				...
				ret_code = caldav.PutCalendarEvent(buffer);
			...
			else if (!strcmp(argument, "3"))     //Delete Event
			{
				...
				sprintf(buffer, "https://apidata.googleusercontent.com%s", caldav.resourceURIs[index - 1].c_str());
				ret_code = caldav.DeleteCalendarEvent(buffer);
